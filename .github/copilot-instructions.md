...
- `chili_app/` is a routed analyst workbench, not a Vite placeholder. Implemented routes include Dashboard, Alert Feed, Investigation Workbench, RAG Chat, and Configuration, plus capability-gated domain pages (Case Management, Policy Intelligence, Housing Executive, Scorecard Run viewer). Knowledge Bases is a library page (`/knowledge-bases`) plus a per-KB workspace (`/knowledge-bases/:kbId/{overview,add,data,runs,settings}`); KB selection there is URL-owned, not store-owned.
