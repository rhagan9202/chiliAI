...
- [ ] Know this if asked: the analytics-review pass (GNN/risk/explainability → alerts) fires **automatically** on records ingest (analytics.34, closed 2026-07-24) — gated by the CMS pack's `records.analytics_trigger` config, throttled to one pass per KB per hour, and focused on the batch's top-3 highest-risk providers. Nothing is manually triggered or staged; alerts, evidence packs, and GNN clusters all come off the same pipeline path any records ingest takes.
