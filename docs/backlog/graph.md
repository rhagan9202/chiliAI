...
- [x] Pyright-strict clean; unit tests cover (a) strict-mode rejection on both adapters, (b) legacy `create_placeholders` mode preserves prior behavior, (c) `upsert_task` surfaces `GraphIntegrityError` and rolls back per `graph.04`. **Deviation:** per-chunk transactional rollback exists (`repository.transaction()` wraps each `_chunk_items` batch); cross-batch task-scoped rollback is NOT implemented — that is graph.04's own (still planned) scope. No unit test proves rollback; only error surfacing/chaining is tested (`test_upsert_task_chains_integrity_error`).
