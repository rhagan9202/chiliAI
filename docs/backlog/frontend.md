...
- **Superseded 2026-07-23 (alerts.36, durable alert read model; live-verification pending Task 6 of `feat/alerts-durable-read-model`):** the U2-era observation that the workbench EVIDENCE tab renders `EmptyState` for real entities because the alert feed's backing store was blind in a fresh deployment (the old API-owned `alert_projection` was only ever populated by test/dev-seed fixtures, per the now-closed `docs/backlog/monitoring.md` story monitoring.02) no longer applies at the code level. `GET /alerts` now reads the durable `alert_history` table directly — the same table Flow 4 writes for every real alert — so any entity with a genuine alert has a real `alert_id`/`evidence_pack_id` for `useEvidencePack(selectedAlert?.evidence_pack_id ?? null, …)` to resolve. This is a code-level supersession only; a full-stack browser pass confirming it against a live alert has not run yet — that is Task 6's job. This story's remaining gap (no independent list/load-by-id entry point, still coupled to the currently-selected alert) is unchanged and still open.
