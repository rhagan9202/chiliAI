...
- Sprint 2026-28 D1 Task 5 (2026-07-23) confirmed this gap live against the CMS demo KB: a natural TN-subset records ingest produces zero alerts. `scripts/demo_cms.sh` / `backend/tools/demo_trigger_analytics.py` now stage an explicit, disclosed `graph.updated` trigger for the demo's top-risk providers as a stand-in. Once this story ships (Flow B firing automatically off a records ingest), that explicit demo trigger step becomes unnecessary and should be removed from `scripts/demo_cms.sh` and the `Makefile`'s `demo-cms` target.
