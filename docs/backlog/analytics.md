...
**Unblocks:** []
**Estimated size:** M
**Done:** 2026-07-24 · records→analytics in-process fan-out (Option B) · `feat/analytics-34-records-analytics-trigger`
...
- Sprint 2026-28 D1 Task 5 (2026-07-23) confirmed this gap live against the CMS demo KB: a natural TN-subset records ingest produces zero alerts. `scripts/demo_cms.sh` / `backend/tools/demo_trigger_analytics.py` staged an explicit, disclosed `graph.updated` trigger for the demo's top-risk providers as a stand-in.
- **DELIVERED 2026-07-24 (`feat/analytics-34-records-analytics-trigger`) — Option B, in-process direct call.** At the tail of `handle_records_ingested`, when the batch's peerstats/timeseries `affected` set yields risk-assessable entities (`assess_entities` now returns the scored `RiskAssessmentResponse` list), the handler calls `handle_graph_updated_for_analytics` directly with an in-memory `GraphUpdatedEvent` carrying inline `upserted_entity_ids` (new optional field on `GraphUpdatedDocumentReference`; `_resolve_upserted_entity_ids` already preferred inline ids). **No event is published** — publishing would run Flow A, which demands storage-key artifacts and would redundantly re-embed entities the records path already indexed; the deleted demo trigger's artifact ceremony (and its `graph-updates/` key that poisoned `chili.embeddings.complete.dlq` on every demo run) is exactly what this avoids. Gated by new `RecordsConfig.analytics_trigger` (`enabled` default False; `max_entities_per_batch` top-N by `overall_score`, default 25; `min_interval_seconds` per-KB `MetricsRecomputeThrottle`, default 600). CMS pack enables top-3/1h. Best-effort wrap mirrors the document dispatch (`_publish_analytics_fanout_failed`, stage `analytics_fanout`) so analytics failures never re-run Flow 1 via retry/DLQ. **Recorded trade-offs:** (1) the throttle is leading-edge — within a window only the first assessable batch fires, so entities landing in later batches wait for the next window/ingest; (2) alert ids embed the batch correlation_id, so successive fired windows re-alert the same top entities as new alert rows (same platform semantics as repeated document re-analysis; feed-level dedup remains `monitoring.21`+ territory). Demo trigger tool, its tests, Makefile wiring, and `demo_cms.sh` block 4.5 deleted. Tests: `backend/tests/agent/test_records_analytics_fanout.py` (records-only KB → clusters + alerts; disabled → nothing; top-N cap; throttle suppression; failure isolation).

### Acceptance Criteria
- [x] A design decision is recorded between the two options below: **Option B chosen** (direct in-process call at the tail of `handle_records_ingested`, with inline entity ids — no publish; rationale in the DELIVERED note above). `records.12`'s publish-the-event toggle was explicitly rejected for Flow B because a published `graph.updated` forces Flow A's storage-key artifact requirements + redundant re-embedding; `records.12` remains open re-scoped to Flow 2/3.
...
- [x] Whichever option is chosen, it is gated so high-volume records feeds are not forced into a GNN/risk recompute on every batch — dedicated per-KB `MetricsRecomputeThrottle` (`min_interval_seconds`) plus a top-N entity cap and a default-off `enabled` flag.
- [x] `backend/records/README.md` and `backend/analytics/README.md` both note that GNN/risk/explainability analytics fire for records-ingested KBs (with the config gate cross-referenced).
- [x] Coverage ≥ 85% on the new/modified handler path (suite aggregate 97%).
...
**As a** fraud analyst scanning the alert feed,
**I need** alerts generated by the analytics/explainability pipeline (Flow B) to show a real entity display label,
**so that** I can recognize which provider/beneficiary/claim an alert is about without opening the entity detail view first.

### Current State
- `alert_history.entity_label` is a real read-model column (migration `0012`, alerts.36/monitoring.02) populated by both alert producers, but both currently fall back to the bare `entity_id`: `_run_explainability_stage` (`backend/agent/coordinator.py`) sets `entity_label=entity_id` with an inline comment explaining why (`entity_label: no display value is cheaply in scope here...`); `MonitoringService.evaluate()` leaves it at the `""` default (a monitoring observation carries no display name at all).
- `build_explanation_context` (`backend/agent/coordinator.py`) already calls `graph_service.get_entity([knowledge_base_id], entity_id)` to build `ExplanationContext` (assigned to the local `focal_entity`, used only to read `.type`) — the fetched `Entity.properties` dict is already in hand in the same function/module, so a real display value (e.g. the domain's configured display-field property, falling back to `entity_id` when absent) is available **without a second graph read**. The blocker noted in the existing code comment (`_run_explainability_stage` is a different function from `build_explanation_context`) is a same-file plumbing gap, not a data-availability gap.
...
- `pytest backend/tests/api/test_read_model_routers.py -q -k alert` — green, confirming `GET /alerts` surfaces the real label end-to-end.
- `pyright` clean, `ruff check --no-cache .` clean.
- Coverage ≥ 85% on the touched `agent/coordinator.py` slice.

### Code touch points
- `backend/agent/coordinator.py` (modify — `build_explanation_context` / `ExplanationContext` / `_run_explainability_stage`)
- `backend/analytics/explainability/models.py` (modify, if `ExplanationContext` needs a new field to carry the label across the two functions)
- `backend/tests/agent/test_alerts_created_graph_flow.py` (modify — new regression test)
