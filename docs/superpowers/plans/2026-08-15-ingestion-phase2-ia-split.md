...
- **Focused run is not URL-owned (§5)** — `RunsSection.tsx` keeps `selectedScoreRunId` in `useState`, where the spec puts the focused run in the URL alongside the focused document. The plan prescribed this as a verbatim carry-over but never listed it as a deferral. Low impact today because `useScoreRuns(…, { limit: 1 })` means there is effectively one run to focus.
- **Retry does not rebuild from current staged state (§2)** — `useDocumentsFlow.tsx`'s upload retry closes over the files captured at submit, so correcting the staged files after a failure and pressing Retry re-uploads the original list. Pre-existing and carried over faithfully from the pre-split manager page, but it is a spec bullet that is neither satisfied nor deferred.
