...
- **Algebra** *(amended 2026-07-15 after Task-1 review found a counterexample)*: right-biased recursive dict union with list/scalar replacement is associative **on type-stable layer stacks** — stacks where no layer changes a key's kind (mapping vs non-mapping) that another layer nests into. Unrestricted layers are NOT associative: a middle layer collapsing a dict to a scalar discards structure that grouping order can resurrect (e.g. `base={"k":{"z":1}}, A={"k":5}, B={"k":{"w":2}}`). Real overlays target the fixed `DomainConfig` schema, so type-stability holds in practice. The property tests (via `hypothesis`, new `[dev]` dependency) prove associativity over generated type-stable stacks plus empty-overlay identity and list-replacement; a deterministic example test pins the type-flip case to its left-to-right (application-order) semantics, and the ADR documents the boundary.
