...
3. **Orchestration:** a `make test-e2e` target that **first** runs `docker compose … down -v` (clean slate; avoids multiple stacks / port collisions), then `up -d --build`, waits for api `/health`, runs Playwright, then `down`. ~~CI runs the same target.~~ **Correction (2026-07-24):** no CI job runs `make test-e2e` or any Playwright suite — `.github/workflows/ci.yml`'s "e2e" mention refers to backend pytest `-m e2e` markers only. `make test-e2e` is a local/pre-merge gate; Playwright-in-CI remains unimplemented.
