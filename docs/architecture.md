...
**Workflow-run lifecycle.** Analytics emits one `RiskScoredEvent` per scored entity, all carrying the *triggering pipeline's* `correlation_id` so the worker resolves the run whose `monitoring` step they complete. Because one run is therefore shared by an unbounded number of them, `risk.scored` is treated as a **fan-out event** rather than a terminal-success event (`_FAN_OUT_EVENT_TYPES`, `agent/workflow_tracking.py`): it completes the `monitoring` step and leaves the run `RUNNING` — the pipeline's own terminal event (`vectors.indexed`) still closes it — and it is not gated by an already-`COMPLETED` run, because the records path publishes its whole fan-out from inside the `records.ingested` handler and every one of them lands after that one-step run has legitimately closed. Re-entering the handler is safe for exactly the idempotency reason above. A `CANCELLED` run still gates them: that is user intent, not bookkeeping.
...
state**. Every KB-scoped page resolves it through one hook
...
**Worker convergence:** the worker (`agent/coordinator.py`) consumes `config.updated` between drain iterations — a rebuild never interleaves with in-flight event handling — and rebuilds its `WorkerDependencies` from the pointer-resolved config. Redelivery is idempotent (`ConfigReloadState` tracks the last applied delivery). On a successful rebuild the replaced dependency set is closed (`WorkerDependencies.close()`, called after the old bus acks its deliveries) — it releases the connection pool, graph driver, vector store client, and both Redis clients so a hot-swap does not leak a full dependency graph's worth of connections; `run_worker` also calls it on shutdown. `close()` best-effort-closes whichever of those five are present (safe on a partially built set) but cannot reach a resource a factory opened and then failed to attach to any `WorkerDependencies` instance before raising.

**Constraint — event transport is swap-invariant:** because the reload signal travels on the pre-swap transport, a pack should **not** change the `events` backend/URI across a hot-swap. The reload signal itself survives (the worker receives it on the old transport, then rebinds to the new one when it rebuilds), but work already queued on the old stream or consumer group has no consumer afterwards, and if the new transport is unreachable the worker keeps its previous dependencies and logs `CONFIG RELOAD FAILED` — leaving the API on the new pack and the worker on the old one.

Since UXA-404, `GET /config/packs` and `POST /config/validate` report each pack's **effective** transport (`PackTransport`) and the Configuration page warns before the swap is confirmed. Effective is the operative word: `resolve_event_bus_settings` falls back to the environment whenever a pack's `events` section is absent or equal to the default `EventBusConfig()`, so of the four shipped packs three pin `redis://redis:6379` explicitly and `medicare_fraud` declares no `events` block at all — yet all four *run* on the environment's transport. A pack can still force in-memory by pinning `backend: in_memory` alongside any other non-default field; the API and worker are separate processes, so each would then build its own in-process bus. None of the shipped packs does.
