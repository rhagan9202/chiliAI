...
`DELETE /knowledgebases/{id}` now executes a complete six-step cascade: graph namespace (`GraphService.delete_knowledge_base`), vector namespace (`VectorService.delete_knowledge_base`), raw records (`RawRecordStore.delete_by_kb`), peerstats derived signals (`DerivedRiskSignalWriterProtocol.delete_by_kb` → `entity_derived_signals`), object-store payloads, and KB repository metadata. If an active workflow run exists for the KB at delete time the API returns 409 to prevent mid-pipeline teardown.
...
| **Delete KB** | `DELETE /knowledgebases/{id}` | Clear graph namespace → clear vector namespace → delete raw records → delete peerstats derived signals → delete object-store payloads → delete KB metadata → publish `kb.delete` | Full 207 cascade implemented; workflow-busy 409 guard prevents deletion during active pipeline run |
