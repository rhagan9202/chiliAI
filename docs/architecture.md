...
`DELETE /knowledgebases/{id}` executes a complete cascade across **every** per-KB durable store, then deletes KB repository metadata. The authoritative ordered step list is `api._kb_cleanup.kb_deletion_steps` (assembled from `api._kb_cleanup.KbDeletionStores`): graph namespace (`GraphService.delete_knowledge_base`), vector namespace (`VectorService.delete_knowledge_base`), `raw_records` + `record_submissions` (`RawRecordStore.delete_by_kb`), peerstats `entity_derived_signals`, `risk_score_history`, `observations`, `alert_history`, `entity_metric_history` + `entity_metrics_current`, `conversations`, `cases`, `policy_items`, evidence packs (each via its repository's `delete_by_kb`), and object-store payloads. Each step runs best-effort (`_run`); any failure surfaces in the 207 body and flags the KB `pending_cleanup`. If an active workflow run exists for the KB at delete time the API returns 409 to prevent mid-pipeline teardown.
...
| **Delete KB** | `DELETE /knowledgebases/{id}` | Cascade-purge every per-KB store (graph, vector, raw_records+submissions, derived signals, risk history, observations, alert history, metrics, conversations, cases, policy, evidence, object store) → delete KB metadata → publish `kb.delete`. Step list: `api._kb_cleanup.kb_deletion_steps` | Full 207 cascade implemented; workflow-busy 409 guard prevents deletion during active pipeline run |
...
Deleting a KB executes a full cascade across every per-KB durable store — graph, vector, `raw_records` + `record_submissions`, peerstats `entity_derived_signals`, `risk_score_history`, `observations`, `alert_history`, `entity_metric_history`/`entity_metrics_current`, `conversations`, `cases`, `policy_items`, evidence packs, and object-store payloads — then KB repository metadata. The ordered step list lives in `api._kb_cleanup.kb_deletion_steps`; adding a new per-KB store is one field on `KbDeletionStores` + one step. A workflow-busy 409 guard prevents deletion while an active pipeline run is in progress. Document-level (single-document) graph/vector cleanup is supported via `delete_by_source_document(kb_id, doc_id)` on both graph and vector protocols.
