...
- **Upload size limits.** File uploads are bounded by the DomainConfig-driven
  ...
  checking is deliberately disabled (`client_max_body_size 0; Helm ingress proxy-body-size "0"`)
  so the config gate is the single authority — a fixed nginx number silently
  contradicted per-pack limits (it defaulted to 1 MB). Multi-GB uploads become
  safe when records.04 (streaming parse) lands; pull-based origins that bypass
  HTTP upload entirely are chartered as records.14–17.
