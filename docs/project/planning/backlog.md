...
| BL-022 | OIDC hardening | REQ-AUTH-001 | P2 | done — Sprint 2026-27 stretch (test-double-verified 2026-07-15) | 0 SP |
...
| BL-048 | Explainability engine: LLM narratives + production SHAP (analytics.13/.14) | REQ-ANALYTICS (explainability) | P1 (sprint 2026-28) | done — Sprint 2026-28 B3 (live-verified 2026-07-23 on `feat/sprint-2026-28-b3-explainability` Task 9: LLM degrade path confirmed end-to-end via the local provider, SHAP attribution rows matching risk-factor contributions, `GET /evidence-packs/{id}` API enrichment, workbench evidence-viewer render, and a scratch KB's KB-delete cascade purging pack rows — all confirmed on TN CMS feeds. The pass surfaced and fixed two defects, both landed on the feature branch: `8326488` (agent: adopt a workflow run created between find and save, closing a check-then-act race on `start_workflow`) and `9e68277` (analytics: degrade section-less LLM narratives to the deterministic fallback — the dev echo provider never emits `## ` headings, so accepting heading-less output as summary-only left every LLM-backed pack with empty `narrative_sections`). Post-merge final review found one further Important-severity gap — a completion opening directly with `## Heading` parsed to an empty `summary` — fixed by `e8f1b30` (also folds `GenerateRequest` construction inside the never-raise guard). Backend gates green throughout: `pytest --cov` 2650 passed/5 skipped/97% coverage, `pyright` 0 errors, `ruff check --no-cache .` clean, contract regen + `npm run build` green) | 0 SP |
| BL-049 | Workbench reshape design pass (IntegrityAI-inspired, design-only) | REQ-UI | P1 (sprint 2026-28) | done — Sprint 2026-28 U1 (design approved 2026-07-23; executed by BL-050) | 0 SP |
| BL-050 | Workbench reshape + analytics surfacing implementation | REQ-UI | P1 (sprint 2026-28) | done — live-verified 2026-07-23 (Sprint 2026-28 U2, `feat/sprint-2026-28-u2-workbench-reshape`: all 9 implementation tasks + Task 10 unit-gate closeout + Task 11 controller live pass. Full Playwright suite 31 passed/2 skipped incl. new tab-aware workbench + triage/narrative-band evidence assertions against the live stack; browser pass confirmed on the TN KB (dossier, Network-tab clusters, Dashboard clusters panel — 4 swatches match); housing pack untouched, zero CMS strings, zero console errors. Final-review closeout fixed a Signals-tab capability-gating gap, added a dossier-header remount key, and dropped two dead `alert.tags` fallbacks; frontend gates green: `npm run test:run` 477/477, `npm run build`, `npm run lint`. See `docs/backlog/frontend.md` story frontend.27 and `docs/project/planning/sprints/2026-28.md`) | 0 SP |
| BL-051 | Complete CMS demo: presenter script, make demo-cms, fraud rule packs, e2e walkthrough | REQ-DEMO | P1 (sprint 2026-28) | done — DoD-verified 2026-07-24 (Sprint 2026-28 D1, `feat/sprint-2026-28-d1-demo`: 2 config-only fraud rule packs firing on real TN data; `make demo-cms` one-command orchestration with 5 readiness probes + disclosed explicit analytics trigger (analytics.34 gap); 3-act presenter script at `docs/demo/` with every claim code-verified; `demo-walkthrough` e2e 7/7 in reference + live modes. Sprint DoD met: fresh `make clean` + `make dev` + `make demo-cms` ran green end-to-end (62m, KB ready / 3 alerts / 4756 clusters / evidence narrative / 2046 policy items), full presenter walkthrough browser-verified zero console errors, full e2e suite 39 passed/1 skipped. Closeout also fixed a real product gap the suite surfaced: Ingestion Studio ignored `?kb=` deep-links. See `docs/project/planning/sprints/2026-28.md`) (the disclosed trigger was the analytics.34 stand-in; analytics.34 shipped 2026-07-24 — the trigger tool is deleted and `make demo-cms` analytics now fire natively on records ingest) | 0 SP |
...
| SAFE-CMS-012 | Entity identity resolution and relationship scoring | SAFE-CMS-SURGE | P1 (PI 3 Sprint 12) | done — identity scoring/projection, Postgres persistence, audited merge/split decisions, API contract, generated frontend schema, and cockpit/dossier identity panel completed with review gates 2026-08-03 | 0 SP |
| SAFE-CMS-013 | Versioned fraud playbooks | SAFE-CMS-SURGE | P1 (PI 4 Sprint 13) | done — versioned fraud playbook config, publication snapshots, KB-scoped API, historical playbook refs, export/import, and cockpit/case UI completed with review gates 2026-08-04 | 0 SP |
| SAFE-CMS-014 | User-authored workflow builder | SAFE-CMS-SURGE | P1 (PI 4 Sprint 14) | done — design, typed models, lifecycle service, KB-scoped API, idempotent run handoff, Postgres persistence and generated contracts landed 2026-08-04 (`d1c2aa6`..`6216c2e`); **execution** (step executor, restricted condition grammar, retries, failure modes, server-side approval gating) landed 2026-08-07. The 2026-08-04 entry's "run handoff" persisted a `QUEUED` run that nothing consumed: `condition`, `retry_policy` and `on_failure` were stored and never evaluated. Approval resume, connector reconciliation and score-replay execution closed 2026-08-07. | 0 SP |
| SAFE-CMS-015 | Capability registry and governed agent tools | SAFE-CMS-SURGE | P1 (PI 4 Sprint 15) | done — registry, KB-scoped browse API, workflow validation, execution envelopes, registry browser and adapters landed 2026-08-05 (`e3fd63b`..`4df497f`); `execute()` dispatch plus **four fail-open authorization branches closed** landed 2026-08-07. The 2026-08-05 "permission checks" read an omitted domain or environment as unrestricted and an empty `required_roles` as everyone — latent only because nothing dispatched. | 0 SP |
| SAFE-CMS-016 | RAG and explanation contract cleanup | SAFE-CMS-SURGE | P1 (PI 4 Sprint 16) | done — streaming/non-streaming RAG filter parity, workflow RAG adapter, shared frontend RAG scope helpers, active-scope UI, and verification merged 2026-08-05 (`7021530`) | 0 SP |
| SAFE-CMS-017 | Pull connectors for CMS-adjacent sources | SAFE-CMS-SURGE | P1 (PI 5 Sprint 17) | done — definitions, sync-run/quarantine tracking, KB-scoped API, frontend management surface and capability status adapter merged 2026-08-05 (`eebd6f5`); **execution** (Postgres persistence, `ConnectorSourceAdapter` + filesystem source, `connector.page.queued` executor, unimplemented-type guards) landed 2026-08-07. The 2026-08-05 entry claimed a "local filesystem adapter" that did not exist: `start_sync` wrote a `queued` row nothing advanced. | 0 SP |
...
## SAFE-CMS Surge Backlog — 2026-08-02 Formalization

These rows formalize the 20-sprint CMS fraud AI surge from
`docs/superpowers/plans/2026-07-30-cms-fraud-ai-safe-agile-20-sprint-surge.md`.
The surge is planned after v1 completion and keeps reusable platform code domain-neutral while CMS labels,
thresholds, typologies, and playbooks live in the CMS domain pack. PI 1 implementation detail is captured in
`docs/superpowers/specs/2026-08-02-safe-cms-pi1-analytics-foundation-design.md`; later PI specs should be
drafted at each PI planning gate before implementation starts.
...
| SAFE-CMS-012 | PI 3 / Sprint 12 | Entity identity resolution and relationship scoring. | SAFE-CMS-001, SAFE-CMS-003. | Implemented 2026-08-03: identity scoring/projection, Postgres identity links, audited review decisions/events, KB-scoped API, generated frontend schema, and cockpit/dossier identity panel with redaction checks. |
| SAFE-CMS-013 | PI 4 / Sprint 13 | Versioned fraud playbooks. | SAFE-CMS-001, SAFE-CMS-004. | Implemented 2026-08-04: PI 4 ADR, config-authored seed playbooks, DB-published immutable snapshots, historical alert/evidence/case refs, KB-scoped API, export/import, and frontend playbook surfaces. |
| SAFE-CMS-014 | PI 4 / Sprint 14 | Permissioned user-authored workflow builder. | SAFE-CMS-002, SAFE-CMS-004, SAFE-CMS-013. | Implemented 2026-08-04: ADR/design, models, lifecycle service, API, run idempotency, Postgres persistence, generated contracts. Execution completed 2026-08-07: `workflow.step.queued` executor, restricted condition grammar with no eval path, per-step retries and failure modes, server-side approval gating, and the run's requesting actor persisted so capability calls authorize as who asked. Live-verified against the running stack. |
| SAFE-CMS-015 | PI 4 / Sprint 15 | Governed capability registry and agent tool permissions. | SAFE-CMS-013, SAFE-CMS-014. | Implemented 2026-08-05: registry/API, workflow validation, execution envelopes, browser UI, adapters. Completed 2026-08-07: authorize-then-dispatch `execute()` with audit on side-effecting calls, and four fail-open authorization branches closed before the dispatcher that would have made them live. Only `connector.sync.status` has a bound executor; the rest report `capability_not_executable`. |
| SAFE-CMS-016 | PI 4 / Sprint 16 | RAG and explanation contract cleanup for cockpit and workflows. | SAFE-CMS-004, SAFE-CMS-005, SAFE-CMS-014. | Implemented 2026-08-05: RAG filter parity, workflow RAG adapter, shared RAG scope helpers, and active-scope UI. |
| SAFE-CMS-017 | PI 5 / Sprint 17 | Pull connectors for CMS-adjacent data sources. | SAFE-CMS-004, SAFE-CMS-013. | Implemented 2026-08-05: connector domain/API, sync and quarantine tracking, frontend management, and status capability adapter. Execution completed 2026-08-07: durable persistence (migration `0025`), source-adapter protocol with a path-guarded filesystem source, page-at-a-time executor with cursor-based idempotency, and 422 rejection of source types and schedule modes that are not implemented. Live-verified against the running stack, including parity with the manual upload path. |
| SAFE-CMS-018 | PI 5 / Sprint 18 | Global KB/domain/readiness controls. | SAFE-CMS-002, SAFE-CMS-003. | Implemented 2026-08-05: readiness aggregation/API, generated frontend client, app-shell workspace control, and live-stack verification. |
| SAFE-CMS-019 | PI 5 / Sprint 19 | Enterprise visual refinement and accessibility pass. | SAFE-CMS-005, SAFE-CMS-006, SAFE-CMS-007. | Implemented 2026-08-05: reusable status language, visual guardrails, and core page status refactors. |
| SAFE-CMS-020 | PI 5 / Sprint 20 | Governance, evals, and release evidence loop. | SAFE-CMS-004, SAFE-CMS-010, SAFE-CMS-014, SAFE-CMS-016. | Implemented 2026-08-05: governance report API/service, dashboard, CMS role-gated config, and e2e smoke merged to `origin/prod`. |
...
- **Status**: **done — Sprint 2026-27 stretch, 2026-07-15** (all 7 AC items shipped on `feat/sprint-2026-27-oidc-hardening`; test-double verification only per product-owner ruling 2026-07-15 — no deferred live-IdP pass exists or is pending; see the deviation note below)
- **Remaining estimate**: 0 SP
- **Acceptance**:
  - [x] JWKS TTL cache keyed by URI — `backend/api/middleware/auth.py` (`JwksCache`)
  - [x] aud/iss/exp + signature validation — `auth.py::decode_token` (`jwt.decode(..., audience, issuer, algorithms=["RS256"])`)
  - [x] id_token routed through the validated decode path in the OIDC callback — `backend/api/routers/auth.py::callback`
  - [x] *(adjacent, landed with ff46080)* Production auth guardrail: incomplete/disabled `AuthConfig` rejected under `CHILI_ENV=staging|production`, at boot and pre-hot-swap — `backend/api/dependencies.py` (`enforce_production_guardrail`), `api/app.py`
  - [x] kid-aware key rotation: `JwksCache.force_refresh` (throttled to once per URI per 30s) triggered by `decode_token` on an unknown `kid`, plus a rotation test matrix — `tests/api/test_auth_middleware.py`
  - [x] nonce generation + validation in the OIDC flow: `/auth/login` generates a `nonce` alongside PKCE `state`/verifier, persisted via `PkceState` (`api/middleware/session_store.py`), verified in `/auth/callback` against the decoded `id_token` — **id_token flows only** (nonce is not an access-token claim by OIDC spec, so the access-token fallback path skips the check)
  - [x] IdP configuration templates: [`docs/auth/idp-templates.md`](../../auth/idp-templates.md) — Keycloak and Okta (scope narrowed from the original 5-IdP/`backend/config/defaults/auth/*.yaml` plan per the design spec's product-owner-approved scope guard; full deviation record in `docs/backlog/_security.md` story _security.01). **Desk-checked against IdP documentation, not verified against a live IdP** — live-IdP verification is the recorded residual follow-up, not a deferred pass owed by this story.
...
| BL-026 | Test coverage for default configs loading | REQ-CONFIG-001 | Sprint 2026-22 | `tests/config/test_loader.py` covers every default yaml (now 4 default packs incl. `department_air_force_housing.yaml`; `medicare_fraud_dev.yaml` retired to `config/overlays/` by BL-044) |
...
- **2026-07-15 BL-022 closeout (Task 4, final task of the story)** — all four implementation/docs tasks landed on `feat/sprint-2026-27-oidc-hardening`: `JwksCache.invalidate_uri`/`force_refresh` with a per-URI 30s forced-refresh throttle (Task 1), kid-aware `decode_token` resolving the signing key from the JOSE header with a rotation test matrix (Task 2), OIDC `nonce` round-trip via `PkceState` with id_token-only validation (Task 3), and worked Keycloak/Okta templates (`docs/auth/idp-templates.md`) plus this closeout pass (Task 4). `docs/backlog/_security.md` story _security.01 flipped to done with two recorded deviations: templates are desk-checked against IdP documentation only, not live-verified (product-owner ruling 2026-07-15 — test-double verification is this stretch's accepted bar, so there is no deferred live-IdP pass outstanding, only a recorded residual follow-up); and nonce validation applies to id_token flows only (id_token-only OIDC-spec claim, so the access-token fallback path skips it by design). Backend gates green in-session (full-suite pytest, `api` package ≥ 85%, pyright 0 errors, ruff clean — see `.superpowers/sdd/task-4-report.md`). **This closes the last open story of Sprint 2026-27** — all committed core (BL-017, BL-044, BL-023) and the BL-022 stretch are now done; the sprint's remaining ~7 SP buffer was not drawn on.
- **2026-07-16 sprint 2026-27 fix-later burn-down (7 tasks, `chore/sprint-2026-27-fixlater-burndown`)** — closed the bulk of the final-review fix-later notes recorded 2026-07-15 against BL-017/BL-044/BL-022 (see `docs/project/planning/sprints/2026-27.md` § Fix-later burn-down for the full per-item mapping): Neo4j relationship `metadata_json` + `updated_at` parity + intra-batch duplicate-id fold (Task 1, graph); `GraphVersionConflictError` now a permanent per-document graph-stage failure (Task 2, agent); `GET /config/validate?with_overlays` dry-runs the merged config (Task 4, api); `AuthConfig.jwks_cache_seconds` wired + malformed-IdP-response 400 + `chili_jwks_forced_refresh_total{outcome}` counter (Task 5, auth); `suppressed_existing_count` receipt field + `ingestion_dedup_suppressed_total{kind="record_row"}` counter (Task 6, records); `config`/`tests/config` added to `tool.pyright.include` with the one surfaced error fixed and `dict[str, Any]` replaced by `pydantic.JsonValue` in `config/overlay.py`/`config/loader.py` (Task 7). Also resolves this file's own "No `scorecards.md` module backlog" carryover flag (§ Carryover notes) — `docs/backlog/scorecards.md` chartered with three `planned` stories. **Survives, explicitly**: live-IdP verification, Auth0/Cognito IdP templates, refresh-token rotation + back-channel logout (all BL-022), the graph.03/graph.04-scoped intra-batch-merge-divergence and change-detection items, and the Neo4j UNWIND-payload-filtering/double-`json.dumps` micro-polish (all BL-017) — none were in this pass's scope. Gates run this session (Docker was down on the implementing machine): `pytest -q -m "not integration"` (2494 passed, 1 skipped, 67 deselected, no `--cov`/`DATABASE_URL` override per controller ruling), `pyright` (0 errors), `ruff check --no-cache .` (clean), frontend `npm run lint` + `npm run test:run` (63 files / 436 tests passed). **Pending on the controller**: live-stack verification (Step 5), the `-m integration` subset, and the full `--cov` run against `chili_test` with the aggregate ≥ 85% gate.
...
- ~~**No `scorecards.md` module backlog** — the new `backend/scorecards/` module has no engineering-story home under `docs/backlog/**`; hardening stories for it (e.g. peer-stats depth, template versioning) currently have nowhere to land. Flag for the module-backlog owner.~~ — **RESOLVED 2026-07-16** (sprint 2026-27 fix-later burn-down, Task 7): [docs/backlog/scorecards.md](../../backlog/scorecards.md) chartered with three `planned` stories (scorecards.01 peer-stats depth hardening, scorecards.02 template versioning, scorecards.03 generation observability); added to `docs/backlog/README.md`'s module-backlog index and status rollup.
...
- [agent.md](../../backlog/agent.md), [analytics.md](../../backlog/analytics.md), [api.md](../../backlog/api.md), [config.md](../../backlog/config.md), [database.md](../../backlog/database.md), [embeddings.md](../../backlog/embeddings.md), [events.md](../../backlog/events.md), [frontend.md](../../backlog/frontend.md), [graph.md](../../backlog/graph.md), [ingestion.md](../../backlog/ingestion.md), [knowledgebases.md](../../backlog/knowledgebases.md), [llm.md](../../backlog/llm.md), [monitoring.md](../../backlog/monitoring.md), [rag.md](../../backlog/rag.md), [records.md](../../backlog/records.md), [scorecards.md](../../backlog/scorecards.md), [shared.md](../../backlog/shared.md), [storage.md](../../backlog/storage.md), [vectorstore.md](../../backlog/vectorstore.md)
