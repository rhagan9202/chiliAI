...
| BL-048 | Explainability engine: LLM narratives + production SHAP (analytics.13/.14) | REQ-ANALYTICS (explainability) | P1 (sprint 2026-28) | done — Sprint 2026-28 B3 (backend implementation; all 8 tasks landed on `feat/sprint-2026-28-b3-explainability`; gates green in-session: `pytest --cov` 2650 passed/5 skipped/97% coverage, `pyright` 0 errors, `ruff check --no-cache .` clean, contract regen + `npm run build` green; full-stack live verification owed to Task 9, reserved for the controller — **not yet run**) | 0 SP |
