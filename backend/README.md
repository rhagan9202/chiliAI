...
- Fresh knowledge bases may not have a registered graph snapshot yet (no entities upserted). Missing snapshots are treated as a controlled skip, not a failed analytics stage, so document/vector Flow A remains quiet and successful while GNN waits for graph data.
