...
- **`api/routers/config.py`** — Viewer-gated reads (`GET /config/domain|features|domain/schema`) plus admin-gated pack management: `GET /config/packs` (discovery + active-pack state) and `POST /config/validate|apply|switch` (dry-run validation; no-restart domain hot-swap via the swap-once-success pipeline — validate + production auth guardrail → persist pointer → atomic DI cache reset → publish `ConfigUpdatedEvent`). Pack references are confined to allow-listed config directories. `GET /config/packs` and `POST /config/validate` also report each pack's **effective** event transport (`PackTransport`), resolved through the public `dependencies.resolve_event_bus_settings` so the UI can warn before a hot-swap abandons queued worker jobs — the environment wins whenever a pack's `events` section is absent or equal to the default, so the declared section would mis-report any pack that omits it (UXA-404). See [`docs/architecture.md` §9.3](../docs/architecture.md#93-active-pack-hot-swap-no-restart-domain-switch).
...
- **`cases/`** — durable, KB-scoped investigation case management (BL-010). `Case` model + `CaseRepository` (in-memory + Postgres adapters, `cases` table via migration `0002_cases`) + `CaseService` with `promote_from_alert` and `attach_alert`. Backs `/cases` CRUD, `POST /cases/promote` (opens a *new* case from an alert) and `POST /cases/{id}/alerts` (adds an alert to a case that already exists — UXA-405; 409 on a re-attach, and it never rewrites `evidence_pack_id`, which records what the case was opened from). All `?knowledge_base_id=`-scoped. See [`cases/README.md`](cases/README.md).
- **`policy/`** — durable, KB-scoped policy intelligence (BL-011). Rule-pack-driven `PolicyItem` generation + analyst triage (accept/reject/defer/escalate-to-case) with persisted `PolicyDisposition`. `PolicyItemRepository` (in-memory + Postgres adapters, `policy_items` table via migration `0003_policy`). Backs `GET /policy/items` (repeated `?status=` for a multi-status queue, `?q=` title search, and a KB-wide `status_counts` facet on the response), `GET /policy/items/{id}`, `POST /policy/items/{id}/triage`. Replaces the old seeded policy-gap surface. See [`policy/README.md`](policy/README.md).
- **`governance/`** — KB-scoped release-readiness reporting for SAFE-CMS-020. `GovernanceReportService` composes published playbook snapshots, approved/draft workflow definitions, and explanation-review outcomes into production version inventory, pending approvals, feedback trends, and release blockers. Backs viewer-gated `GET /knowledgebases/{knowledge_base_id}/governance/report`.
...
- **`analytics/explainability/`** also owns the **evidence-pack repository** (BL-005): real packs are extracted in the worker (`graph.get_subgraph` + risk factors → `ExplanationContext` → `ExplainabilityService`), persisted to an object-store `EvidencePackRepository`, and served by `GET /evidence-packs/{id}` (KB-scoped) — replacing the seeded `ApiState` evidence read model. `GET /evidence-packs/{id}/export?format=json|markdown` renders a downloadable copy via `analytics/explainability/export.py`; rendering lives in this module rather than `api/` because it owns evidence packs, and on demand rather than stored because the pack is already durable and the render is a deterministic projection of it (UXA-405). `ExplainabilityService` composes two injected, config-selected seams (BL-048, Sprint 2026-28 B3): a `NarrativeGeneratorProtocol` (`DeterministicNarrativeGenerator` default, or `LlmNarrativeGenerator` degrading to it) and a `FeatureAttributorProtocol` (`NoopFeatureAttributor` default, or `ShapRiskAttributor` attributing the linear risk composite) — both never raise, degrading with a WARNING log instead. Persisted packs carry the results as `narrative_sections`/`attribution`, served through `EvidencePackResponse`. See "Analytics Runtime Notes" below and [`analytics/README.md`](analytics/README.md) § Explainability narrative + attribution seams.
- **`api/middleware/`** — Metrics, auth, and RBAC middleware with route-level policy enforcement and auth-enabled startup audit. `GET /metrics` here exposes the API process's own `prometheus_client` registry only — it is a separate registry from the worker's, so a full scrape needs both endpoints. `JwksCache` (`auth.py`) resolves signing keys by `kid`: an unknown `kid` triggers a forced JWKS refetch throttled to once per URI per 30 seconds (BL-022), so an IdP key rotation recovers without a restart or waiting out the TTL. The OIDC login/callback flow (`routers/auth.py` + `routers/_oidc_client.py`) round-trips a `nonce` alongside the PKCE verifier and validates it against the decoded `id_token` (id_token flows only — the access-token fallback path has no nonce claim to check). `nonce` binds the id_token to the *authorization request*, not to the browser, so `login()` also sets a short-lived `chiliai_login_state` cookie (HttpOnly, `SameSite=Lax`, hardened the same as `chiliai_session`) and `callback()` requires it to constant-time-match the `state` query param before it will even look up the PKCE record — without that check, an attacker can start a login, capture their own `code`+`state`, and induce a victim's browser into the callback to log the victim into the attacker's account (login CSRF / session fixation). The cookie is cleared on every callback exit path, success or failure. See [`docs/auth/idp-templates.md`](../docs/auth/idp-templates.md) for worked Keycloak/Okta `AuthConfig` YAML and IdP-side setup steps (desk-checked, not live-verified — see the doc's header), and [`docs/wiki/flows/auth-flow.md`](../docs/wiki/flows/auth-flow.md) for the full step-by-step flow.
...
├── governance/      # KB-scoped release readiness over playbooks, workflows, reviews
...
- GNN analytics currently fire on document-driven `graph.updated` events only (`agent.coordinator.handle_graph_updated_for_analytics`, Flow B) — `graph.service.upsert_records_graph` still publishes no `GraphUpdatedEvent` for structured-records ingestion; since analytics.34 (2026-07-24) `handle_records_ingested` instead ends with a best-effort in-process Flow B fan-out (GNN → risk → explainability → alerts) gated by `DomainConfig.records.analytics_trigger` (default off; the CMS pack enables it), throttled per KB, capped to the batch's top-N entities by risk score.
