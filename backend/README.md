...
- **`database/`** — Postgres + TimescaleDB connection provider, `DatabaseConfig`-driven backend selection, and Alembic-managed schema (thirteen persistence tables across migrations `0001`-`0009`, including the durable per-document ingestion status projection `source_document_status`). Infrastructure only — no domain logic.
...
- **`api/middleware/`** — Metrics, auth, and RBAC middleware with route-level policy enforcement and auth-enabled startup audit. `GET /metrics` here exposes the API process's own `prometheus_client` registry only — it is a separate registry from the worker's, so a full scrape needs both endpoints.
