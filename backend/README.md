...
- **`database/`** — Postgres + TimescaleDB connection provider, `DatabaseConfig`-driven backend selection, and Alembic-managed schema (fourteen persistence tables across migrations `0001`-`0010`, including the durable per-document ingestion status projection `source_document_status` and the durable DLQ ledger `event_dlq`, BL-023). Infrastructure only — no domain logic.
