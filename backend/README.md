...
- **`database/`** — Postgres + TimescaleDB connection provider, `DatabaseConfig`-driven backend selection, and Alembic-managed schema (fifteen persistence tables across migrations `0001`-`0011`, including the durable per-document ingestion status projection `source_document_status`, the durable DLQ ledger `event_dlq` (BL-023), and the persisted self-history anomaly ledger `timeseries_anomalies` (migration `0011`, BL-047)). Infrastructure only — no domain logic.
