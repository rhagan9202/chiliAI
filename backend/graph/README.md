...
- **`integrity_mode`** (`"strict"` default, or `"create_placeholders"`) — relationship upserts only: `strict` raises `GraphIntegrityError` if any relationship references a `source_id`/`target_id` not already present in the KB; `create_placeholders` is implemented on both adapters as "skip the strict endpoint-existence check" (legacy behavior preserved).

**Status by adapter:** `InMemoryGraphRepository.upsert_entities` (BL-017 Task 2) and `InMemoryGraphRepository.upsert_relationships` (BL-017 Task 3) implement merge/version/integrity semantics in full. `Neo4jGraphRepository.upsert_entities` implements merge/version semantics via an atomic read-modify-write — one `MATCH` read followed by one `MERGE` write, both run through the caller's active transaction when `repository.transaction(kb)` is open (BL-017 Task 4). `Neo4jGraphRepository.upsert_relationships` implements the same merge/version semantics plus strict-mode endpoint verification — a `MATCH` read of both endpoint `entity_id`s before the write, raising `GraphIntegrityError` on any miss, then `MATCH (source) MATCH (target) MERGE (source)-[r]->(target)` instead of `MERGE`-creating either endpoint (BL-017 Task 5). Under the default `strict` `integrity_mode`, both adapters reject dangling endpoints. When `create_placeholders` is enabled, the adapters diverge (pre-existing legacy divergence, kept deliberately): `InMemoryGraphRepository` stores the dangling relationship without creating entity records, while `Neo4jGraphRepository` uses `MERGE` endpoint clauses which create placeholder entity nodes (visible via `get_entities`).

**Deviations from the graph.01/graph.02 story text (2026-07-14):** `integrity_mode` travels as a field on `GraphUpsertOptions` (passed through `GraphBuildTask.upsert_options`) rather than as a separate `GraphService` keyword argument — this keeps a single options object threading through `upsert_task`, `upsert_records_graph`, and both adapters instead of two independently-plumbed parameters. `merge_properties` is a **shallow** dict merge (top-level keys only), not the story text's "deep-merge" — a shallow merge is sufficient for the flat `properties`/`metadata` dicts entities and relationships actually carry, and matches the pre-existing `update_entity_properties` merge behavior it now shares an implementation pattern with.
