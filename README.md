...
> **Current state**: chiliAI is an active local-development prototype. The backend includes the FastAPI gateway, domain config, event bus, ingestion, graph, vector, embeddings, LLM, RAG, analytics, monitoring, storage, auth/RBAC middleware, config-driven adapter selection, CI, and worker orchestration with extensive tests. The 2026-08 SAFE-CMS surge added durable audit logging, versioned fraud playbooks, workflow definitions, a capability registry, pull-connector definitions, readiness aggregation and governance/eval runs — implementation depth varies sharply across those, so verify against code before relying on any of them (see `docs/ledger/module-map.md`). The frontend includes a routed analyst workbench shell with Dashboard, a Knowledge Bases area (library of KB cards + a per-KB workspace with Overview/Add data/Data/Runs/Settings sections), Alert Feed, Investigation Workbench, Case Management, Policy Intelligence, Governance, Housing Executive, Scorecard Run viewer, RAG Chat, and a Configuration page with the Config Manager (pack switcher, dry-run validation, hot-swap apply; raw pack save is still a gap). See [`docs/architecture.md` §14.3](docs/architecture.md#143-current-state-vs-target) for current state vs. target.
